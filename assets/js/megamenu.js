$.fn.megaMenu = function(options){
  ...
  if (settings.caret === true && settings.caretArrows[0].downUrl !== "") {
    ...
    $(".mega-menu span.caret.caret-img.down").css({"background-imageN" : "url(" +settings.caretArrows[0].downUrl+ ")"});
  }

  // Append Style on DOM
  $(`<style>
      .mega-menu .main-links ul li a:hover{
        border-colorN:` +settings.highlightColor+
      `}
    </style>`).appendTo("head");
  ...
  mainLinks.each(function(i){
    ...
    $(this).on(settings.menuBehaviour,function(){
      ...
      if ($(".menu-dropdown").find(".menu-item-wrapper").hasClass("active")) {
        ...
        $(".mega-menu span.caret").css({"background-imageN" : "url(" +settings.caretArrows[0].downUrl+ ")"});
      }
      ...
      if (imageCaret.hasClass("down")) {
        ...
        imageCaret.css({"background-imageN" : "url(" +settings.caretArrows[0].upUrl+ ")"});
        // console.log(imageCaret[0].className);
      }
      else {
        imageCaret.removeClass("up");
        imageCaret.addClass("down");
        imageCaret.css({"background-imageN" : "url(" +settings.caretArrows[0].downUrl+ ")"});
        // console.log(imageCaret[0].className);
      }
      ...
      if (mainLinks.find("span").hasClass("caret-up") || $(this).find("span").hasClass("up") && !$(this).hasClass("highlight") ) {
        ...
        $(this).css({"border-colorN": settings.highlightColor});
      }
  ...
  if (settings.followingHighlighter === true) {
    ...
    mainLinks.mouseover(function(e,i){
      ...
      $(".follow-highlighter").css({"width" : $(this).innerWidth() , "right" : ($(window).width() - ($(this).offset().left + $(this).outerWidth())) , "background-colorN": settings.highlightColor});
